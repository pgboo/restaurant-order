  ...
  <div>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑' : '新增'" width="400px">
      <el-form :model="form" label-width="60px">
        ...
        <el-form-item label="折扣"><el-input-number v-model="form.discount" :min="0.01" :max="1" :step="0.05" :precision="2" /></el-form-item>
      </el-form>
...
const loadData = async () => {
  const res = await memberLevelApi.list({ page: page.value, pageSize: 10 })
...
const handleSave = async () => {
  if (isEdit.value) await memberLevelApi.update(form.value); else await memberLevelApi.add(form.value)
  ElMessage.success('操作成功'); dialogVisible.value = false; loadData()
}
const handleDelete = async (id) => {
  await ElMessageBox.confirm('确定删除？')
  await memberLevelApi.delete(id); ElMessage.success('已删除'); loadData()
}
onMounted(loadData)
</script>