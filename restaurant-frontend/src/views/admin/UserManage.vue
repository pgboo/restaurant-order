...
const loadData = async () => { const res = await usersApi.list({ page: page.value, pageSize: 10, username: search.value }); list.value = res.data.list; total.value = res.data.total }
const handleDelete = async (id) => { await ElMessageBox.confirm('确定删除？'); await usersApi.delete(id); ElMessage.success('已删除'); loadData() }
onMounted(loadData)
</script>