...
const loadData = async () => {
  const res = await wupinTypeApi.list({ page: page.value, pageSize: 10 })
...
const handleSave = async () => {
  if (isEdit.value) await wupinTypeApi.update(form.value); else await wupinTypeApi.add(form.value)
  ElMessage.success('操作成功'); dialogVisible.value = false; loadData()
}
const handleDelete = async (id) => {
  await ElMessageBox.confirm('确定删除？')
  await wupinTypeApi.delete(id); ElMessage.success('已删除'); loadData()
}
onMounted(loadData)
</script>