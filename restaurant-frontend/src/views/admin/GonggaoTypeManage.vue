...
import { gonggaoTypeApi } from '../../api'
...
const loadData = async () => {
  const res = await gonggaoTypeApi.list({ page: page.value, pageSize: 10 })
...
const handleSave = async () => {
  if (isEdit.value) await gonggaoTypeApi.update(form.value); else await gonggaoTypeApi.add(form.value)
  ElMessage.success('操作成功'); dialogVisible.value = false; loadData()
}
const handleDelete = async (id) => {
  await ElMessageBox.confirm('确定删除？')
  await gonggaoTypeApi.delete(id); ElMessage.success('已删除'); loadData()
}
onMounted(loadData)
</script>