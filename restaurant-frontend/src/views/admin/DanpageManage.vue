...
import { danpageApi, danpageTypeApi } from '../../api'
...
const loadData = async () => { const res = await danpageApi.list({ page: page.value, pageSize: 10 }); list.value = res.data.list; total.value = res.data.total }
const loadTypes = async () => { const res = await danpageTypeApi.all(); types.value = res.data }
...
const handleSave = async () => {
  if (isEdit.value) await danpageApi.update(form.value); else await danpageApi.add(form.value)
  ElMessage.success('操作成功'); dialogVisible.value = false; loadData()
}
const handleDelete = async (id) => { await ElMessageBox.confirm('确定删除？'); await danpageApi.delete(id); ElMessage.success('已删除'); loadData() }
